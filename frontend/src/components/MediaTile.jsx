...
export default function MediaTile({ entry }) {
  ...
  return (
    <div
      className={`group relative w-full aspect-8/11 rounded-md overflow-hidden bg-surface-2 border-b-2 border-r-2 ${borderColor}`}
    >
      ...
      {/* Media type badge */}
      <span
        className={`absolute top-0 left-0 rounded-br-md px-2 py-0.5 text-[10px] sm:text-xs text-white flex items-center justify-center ${
          entry.media_type === 'game'
            ? 'bg-[#0070CC]'
            : entry.media_type === 'movie'
              ? 'bg-[#B20710]'
      ...
      {entry.rating && (
        <span className="absolute bottom-0 left-0 bg-black/40 rounded-tr-md px-2 py-0.5 text-white text-[10px] sm:text-xs font-bold flex items-center gap-1">
          <Star
            size={10}
            className="sm:hidden"
            fill="#FFB800"
